...
static Bool 		clo_pso					=False;
static Bool 		clo_sbit				=False;
static Bool 		clo_multi				=False;
static Bool 		clo_pscv				=False;
...
//
static UInt cstoreCount 						= 0;
static UInt bstoreCount 						= 0;
static UInt gstoreCount 						= 0;
static UInt gistoreCount 						= 0;
static UInt rstoreCount 						= 0;
static UInt lstoreCount 						= 0;
static UInt u1storeCount 						= 0;
static UInt u8storeCount 						= 0;
static UInt u16storeCount 						= 0;
static UInt u32storeCount 						= 0;
static UInt u64storeCount 						= 0;
static UInt f32storeCount 						= 0;
static UInt f32istoreCount 						= 0;
static UInt f64storeCount 						= 0;
static UInt f64istoreCount 						= 0;
static UInt v128storeCount 						= 0;
static UInt v256storeCount 						= 0;

static UInt UnOpCount 						= 0;
static UInt BinOpCount 						= 0;
static UInt TriOpCount 						= 0;
static UInt QuOpCount 						= 0;

static UInt dBinOpCount 						= 0;
static UInt iBinOpCount 						= 0;
static UInt uBinOpCount 						= 0;
...
static Int cell_bit=			0;
static Int pos=					0;
static Int status=				0;
static Int bstatus=				0;
static Int pscfile				;
static HChar pscvfname[256]		;
static SysRes pscfileRes		;
static Int pscvcount=			0;
static mpfr_sign_t  			_mpfr_sign;
static mpfr_exp_t  				_mpfr_exp;
static mpfr_exp_t  				exptmp;
static mpfr_prec_t				_mpfr_prec;
static mp_limb_t 				_mpfr_dl;
static mp_limb_t 				dtemp;
static mp_limb_t 				_mpfr_d0;
static mp_limb_t				_mpfr_hl;
static mp_limb_t				ltemp;
static mp_limb_t				_mpfr_dl0;

static mp_exp_t 				mpexp;
...
static Bool fd_process_cmd_line_options(const HChar* arg) {
	if VG_BINT_CLO(arg, "--precision", clo_precision, MPFR_PREC_MIN, MPFR_PREC_MAX) {}
   	...
   	else if VG_BOOL_CLO(arg, "--pso", clo_pso) {}
   	else if VG_BOOL_CLO(arg, "--sbit", clo_sbit) {}
   	else if VG_BOOL_CLO(arg, "--multi", clo_multi) {}
	else if VG_BOOL_CLO(arg, "--pscv", clo_pscv) {}
	else
		return False;
...
"    --pso=no|yes        [no]\n"
"    --sbit=no|yes       no: rounding result is replaced by origin value, yes:roungding result with shadow value [no]\n"
"    --multi=no|yes      deal with multi  [no]\n"
	);
}
...
static VgHashTable* bitWrtmp 		=NULL;
...
ShadowValue* initShadowValue(UWord key) {
	...
	mpfr_init(sv->value);
//	mpfr_init(sv->addArg1);
	mpfr_init_set_d(sv->addArg1,0.0,STD_RND); 
	mpfr_init_set_d(sv->addArg2,0.0,STD_RND);
	mpfr_init_set_d(sv->mulArg1,0.0,STD_RND); 
	mpfr_init_set_d(sv->mulArg2,0.0,STD_RND);
//	mpfr_init(sv->addArg2);
...
void copyShadowValue(ShadowValue* newSv, ShadowValue* sv) {
	...
	mpfr_set(newSv->addArg1,sv->addArg1,STD_RND);
	mpfr_set(newSv->addArg2,sv->addArg2,STD_RND);
	if(status != 0){
	mpfr_set(newSv->mulArg1,sv->mulArg1,STD_RND);
	mpfr_set(newSv->mulArg2,sv->mulArg2,STD_RND);
	}
...
static  Bool checkPSMT(ShadowValue* arg1, mpfr_t arg2)
{
	if(arg1 == NULL)
	{
		status =0;
	...
	if(status == 1)
	{
		if(mpfr_cmp(arg1->mulArg2,arg2) == 0)
			status = 2;
		else{
			status = 0;
			return False;
	...
	{
		mpfr_add(arg2,arg2,arg1->mulArg2,STD_RND);
		status = 0;
		if(mpfr_cmp(arg1->value,arg2) == 0)
		return True;
		else
		{
			return False;
...
static VG_REGPARM(2) void processUnOp(Addr addr, UWord ca) {
	if (!clo_analyze) return; //analyze all
...
static VG_REGPARM(2) void processBinOp(Addr addr, UWord ca) {
	if (!clo_analyze) return; //analys all
	Int constArgs = (Int)ca;
	...
	if (constArgs & 0x1) {
		...
		arg1tmp = getTemp(binOpArgs->arg1);
		if (arg1tmp) {
	...
	if (constArgs & 0x2) {
		...
		arg2tmp = getTemp(binOpArgs->arg2);
		if (arg2tmp) {
	...
	float fltMedian,fltConst;
	double dMedian,dConst;
	...
	switch (binOpArgs->op) {
		case Iop_Add32F0x4:
		case Iop_Add64F0x2:
			mpfr_set(res->addArg1,arg1tmpX,STD_RND);
			mpfr_set(res->addArg2,arg2tmpX,STD_RND);
			mpfr_add(res->value, arg1tmpX, arg2tmpX, STD_RND);
			canceled = getCanceledBits(&(res->value), &(arg1tmpX), &(arg2tmpX));
			break;
		case Iop_Sub32F0x4:
		case Iop_Sub64F0x2://fix method1,origin to shadow, fix method2,to remap shadow 
			if(arg1tmp && (mpfr_get_exp(arg2tmpX)>10))
			{
				if((mpfr_cmp(arg2tmpX,arg1tmp->addArg1) == 0))//&&(mpfr_cmp(arg1tmp->addArg1,arg1tmp->addArg2))
				{
					// if(mpfr_cmp_d(arg1tmp->addArg2,0.0) < 0)
						// mpfr_sub(res->value,arg2tmpX,arg1tmp->addArg2,STD_RND);
					// else if (mpfr_cmp_d(arg1tmp->addArg2,0.0) > 0)
						// mpfr_add(res->value,arg2tmpX,arg1tmp->addArg2,STD_RND);
					// if(mpfr_cmp(res->value,arg1tmp->value)==0)
					isRounding = True;
					const HChar* lastOperation = VG_(describe_IP)(res->origin, NULL);
					VG_(sprintf)(formatBuf, "Rounding: %s\n", lastOperation);
					my_fwrite(pscfile, (void*)formatBuf, VG_(strlen)(formatBuf));
					fwrite_flush();
					// VG_(umsg)("(Rounding) Last operation: %s\n", lastOperation);
				}
				else if(mpfr_cmp(arg2tmpX,arg1tmp->addArg2) == 0)
				{
					mpfr_add(res->value,arg2tmpX,arg1tmp->addArg1,STD_RND);
					if(mpfr_cmp(res->value,arg1tmp->value)==0)
					isRounding = True;
					const HChar* lastOperation = VG_(describe_IP)(res->origin, NULL);
					VG_(sprintf)(formatBuf, "Rounding: %s\n", lastOperation);
			...
			if(isRounding)
			{
				...
				{
					if (isOpFloat(binOpArgs->op)) {
						mpfr_set_flt(res->value, binOpArgs->orgFloat, STD_RND);
					} else {
						mpfr_set_d(res->value, binOpArgs->orgDouble, STD_RND);
					}
				}
				else{
					if (isOpFloat(binOpArgs->op)) {
						fltMedian = mpfr_get_flt(arg1tmpX,STD_RND)-mpfr_get_flt(arg2tmpX,STD_RND);
						mpfr_set_flt(res->value,fltMedian , STD_RND);
					}
					else
					{
						dMedian = mpfr_get_d(arg1tmpX,STD_RND) - mpfr_get_d(arg2tmpX,STD_RND);
						mpfr_set_d(res->value, dMedian, STD_RND);
					}
				}
				mpfr_set_d(arg1tmp->addArg1,0.0,STD_RND);
				mpfr_set_d(arg1tmp->addArg2,0.0,STD_RND);
				canceled = getCanceledBits(&(res->value), &(arg1tmpX), &(arg2tmpX));
			}
			else if(checkPSMT(arg1tmp,arg2tmpX))
			{
				// const HChar* lastOperation = VG_(describe_IP)(arg1tmp->origin, NULL);
				// VG_(umsg)("(Multiplication) Last operation: %s\n", lastOperation);
				const HChar* lastOperation = VG_(describe_IP)(arg1tmp->origin, NULL);
				// VG_(get_and_pp_StackTrace)(VG_(get_running_tid)(), 16);
				VG_(sprintf)(formatBuf, "Multiplication: %s\n", lastOperation);
				my_fwrite(pscfile, (void*)formatBuf, VG_(strlen)(formatBuf));
				fwrite_flush();
				
				mpfr_sub_d(arg3tmpX,arg1tmp->mulArg1,1.0,STD_RND);
				arg3tmpX->_mpfr_exp=(int)(clo_precision%2==0 ? clo_precision/2:clo_precision/2+1)+1;
				mpfr_add_d(arg3tmpX,arg3tmpX,1.0,STD_RND);
				mpfr_mul(arg3tmpX,arg3tmpX,arg1tmp->mulArg2,STD_RND);
				mpfr_sub(arg1tmpX,arg3tmpX,arg1tmp->mulArg2,STD_RND);
				mpfr_sub(res->value,arg3tmpX,arg1tmpX,STD_RND);
			}
			else
			{
				if(mpfr_cmp_d(arg2tmpX,0.0) < 0)
				{
					mpfr_set(res->addArg1,arg1tmpX,STD_RND);
					mpfr_set(res->addArg2,arg2tmpX,STD_RND);	
				} 
		...
		case Iop_Mul32F0x4:
			if(clo_multi)
			{
				if(mpfr_cmp_d(arg1tmpX,(0x1<<12)+1) == 0)
				{
					// mpfr_set_d (arg1tmpX,(0x1<<(int)(clo_precision%2==0 ? clo_precision/2:clo_precision/2+1))+1.0,STD_RND);
					// VG_(printf)("CNarg1tmpX:%.30f,%.30f\n",mpfr_get_d(arg1tmpX,STD_RND),mpfr_get_d(arg2tmpX,STD_RND));
					mpfr_set(res->mulArg1,arg1tmpX,STD_RND);
					mpfr_set(res->mulArg2,arg2tmpX,STD_RND);
					
				}
				else if(mpfr_cmp_d(arg2tmpX,(0x1<<12)+1) == 0)
				{
					
					// mpfr_set_d (arg2tmpX,(0x1<<(clo_precision%2==0 ?clo_precision/2:clo_precision/2+1))+1,STD_RND);
					// VG_(printf)("CNarg2tmpX:%.30f,%.30f\n",mpfr_get_d(arg1tmpX,STD_RND),mpfr_get_d(arg2tmpX,STD_RND));
					mpfr_set(res->mulArg1,arg2tmpX,STD_RND);
					mpfr_set(res->mulArg2,arg1tmpX,STD_RND);
				}
				status = 1;
			}
		...
		case Iop_Mul64F0x2:
			if(clo_multi)
			{
				if(mpfr_cmp_d(arg1tmpX,(0x1<<27)+1) == 0)
				{
					mpfr_set(res->mulArg1,arg1tmpX,STD_RND);
					mpfr_set(res->mulArg2,arg2tmpX,STD_RND);
					// mpfr_set_d (arg1tmpX,(0x1<<(int)(clo_precision%2==0 ? clo_precision/2:clo_precision/2+1))+1.0,STD_RND);
				...
				else if(mpfr_cmp_d(arg2tmpX,(0x1<<27)+1) == 0)
				{
					mpfr_set(res->mulArg1,arg2tmpX,STD_RND);
					mpfr_set(res->mulArg2,arg1tmpX,STD_RND);
					// mpfr_set_d (arg2tmpX,(0x1<<(clo_precision%2==0 ?clo_precision/2:clo_precision/2+1))+1,STD_RND);
		...
		case Iop_Max64F0x2:
			...
			break;
		case 5770:
		// VG_(printf)("5770:%.30f,%.30f\n",mpfr_get_d(arg1tmpX,STD_RND),mpfr_get_d(arg2tmpX,STD_RND));
		if(mpfr_nan_p(arg1tmpX))
		{
			mpfr_abs(res->value,arg2tmpX,STD_RND);
		}
		else if (mpfr_zero_p(arg1tmpX))
		{
			mpfr_set_ui(res->value,arg2tmpX->_mpfr_sign,STD_RND);
		}
		else if (mpfr_nan_p(arg2tmpX))
		{
			mpfr_abs(res->value,arg1tmpX,STD_RND);
		}
		else 
		{
			mpfr_set_ui(res->value,arg1tmpX->_mpfr_sign,STD_RND);
		}

		break;
		case 5771:
		// VG_(printf)("5771:%.30f,%.30f\n",mpfr_get_d(arg1tmpX,STD_RND),mpfr_get_d(arg2tmpX,STD_RND));
		
		
		if(mpfr_cmp_d(arg2tmpX,1.0) == 0)
		{
			mpfr_abs(res->value,arg1tmpX,STD_RND);
		}
		else 
		{
			mpfr_abs(res->value,arg1tmpX,STD_RND);
			mpfr_neg(res->value,res->value,STD_RND);
		}
		HT_PSFB* psfbTemp = VG_(malloc)("psfb.ht.1", sizeof(HT_PSFB));
		HT_PSFB* curpsfbTemp = VG_(HT_lookup)(bitWrtmp, addr);
		// ShadowValue* av = VG_(HT_lookup)(globalMemory, addr);
		if(!curpsfbTemp){
			// VG_(printf)("nofound!\n");
		// VG_(printf)("%X\n",addr);
		psfbTemp->tempid = addr;
		mpfr_init2(psfbTemp->value,clo_precision);
		mpfr_set(psfbTemp->value,res->value,STD_RND);
		VG_(HT_add_node)(bitWrtmp, psfbTemp);
		}
		break;
		case 5772:
			//  VG_(printf)("5772:%.30f,%.30f\n",mpfr_get_d(arg1tmpX,STD_RND),mpfr_get_d(arg2tmpX,STD_RND));
			// VG_(HT_add_node)(bitWrtmp, bit);
			 if(mpfr_zero_p(arg1tmpX))
			 mpfr_neg(res->value,arg2tmpX,STD_RND);
			 else
			 mpfr_neg(res->value,arg1tmpX,STD_RND);
			break; 
		// case 5761:
		// 	VG_(printf)("5761:%.30f,%.30f\n",mpfr_get_d(arg1tmpX,STD_RND),mpfr_get_d(arg2tmpX,STD_RND));
		// 	// VG_(get_and_pp_StackTrace)(VG_(get_running_tid)(), 16);
		// 	break;
		default:
...
static VG_REGPARM(3) void processLoad(UWord tmp, Addr addr,UWord isDouble) {
	Int isD = (Int) isDouble;
	if (!clo_analyze) return;
	ShadowValue* av = VG_(HT_lookup)(globalMemory, addr);
	if(isD) //range is decided by the pscv  
	{
		Double* orgDb = (Double*)addr;
		UInt *orgInt = (UInt*)addr;
		UInt tempInt = *(orgInt+1);
		tempInt = tempInt & 0x7fffffff;
		if((tempInt > 0x3c000000) && (tempInt < 0x41000000) && (!av))
		{
			av = initShadowValue((UWord)addr);
			if (clo_simulateOriginal) {
				mpfr_set_prec(av->value,53);
			}
			
			if(clo_pscv)
			{
				mpfr_set_d(av->value,*orgDb,STD_RND);
				if(set_mpfr(av, *orgDb))
				{
					VG_(sprintf)(formatBuf, "(PSCV):  %X 0x%X 0x%X %f\n", addr, *(orgInt+1),*orgInt,*orgDb);
					my_fwrite(pscfile, (void*)formatBuf, VG_(strlen)(formatBuf));
					fwrite_flush();
				}
			}
			...
			av->Org.db = *orgDb;
			av->orgType = Ot_DOUBLE;
			av->active = True;
			av->isConst =True;
			av->opCount = 0;
			VG_(HT_add_node)(globalMemory, av);
...
static void instrumentLoad(IRSB* sb, IRTypeEnv* env, IRStmt* wrTmp) {
	...
	Int isDouble = 0;
	if ((load->Iex.Load.addr->tag != Iex_Const)&&(load->Iex.Load.addr->tag != Iex_RdTmp)) {
		return;
	}
	switch (load->Iex.Load.ty) {
			case Ity_I64:
			case Ity_F64:
			case Ity_V128:
				isDouble = 1;
	...
	isDouble = 0;
	IRExpr** argv = mkIRExprVec_3(mkU64(wrTmp->Ist.WrTmp.tmp), load->Iex.Load.addr,mkU64(isDouble));
	IRDirty* di = unsafeIRDirty_0_N(3, "processLoad", VG_(fnptr_to_fnentry)(&processLoad), argv);
	addStmtToIRSB(sb, IRStmt_Dirty(di));
}
static VG_REGPARM(3) void processStore(Addr addr, UWord t, UWord isFloat) {
	...
	UInt* orgUi = (UInt*)addr;
	Double* orgDb = (Double*)addr;
	// VG_(printf)("1895 t %X %X %X %lf\n",addr  , *(orgUi) ,*(orgUi+1),*orgDb);
	if(isFloat <= 1)
	{
		...
		/* check if this memory address is shadowed */
			...
			if(curpsfbTemp)
			{
				VG_(printf)("1828 %lf %lf\n ",mpfr_get_d(curpsfbTemp->value,STD_RND),*orgDb );
			}
			ShadowValue* av = getTemp(tmp);
			if (av) {
				if (currentVal) {
					double pscfbsv = mpfr_get_d(av->value,STD_RND);
					UInt* pscfbUi = (UInt*)(&pscfbsv);
					if( ((*(pscfbUi+1)) ^ (*(orgUi+1))) == 0x80000000)
					{
						// VG_(printf)("1838 %lf %lf\n ",mpfr_get_d(av->value,STD_RND),*orgDb );
						mpfr_neg(av->value,av->value,STD_RND);
					}
					res = currentVal;
					copyShadowValue(res, av);
					res->active = True;
...
//				res->isConst = False;
				...
				if(((Int)isFloat==0) ){
					...
					if(curpsfbTemp)
					{
						VG_(printf)("1871 %lf %lf\n ",mpfr_get_d(curpsfbTemp->value,STD_RND),*orgDb );
					}
					// if((Int)(*orgDb ) == (Int)(*orgDb + 6755399441055744.0))
					if(((*(orgTemp+1)>0x100000)||*(orgTemp+1)==0)&&(*(orgTemp+1)<0xFFFFFFFF))
					{
						if (currentVal) 
						{
							res = currentVal;
							...
							mpfr_set_d(res->value,*orgDb,STD_RND);
							
							res->Org.db=*orgDb;
							res->opCount=currentVal->opCount + 1;
							res->orgType= Ot_INVALID;
							res->cancelOrigin=addr;
							res->canceled=argCanceled;
							res->active = True;
							res->isConst=True;
							if(currentVal->opCount == 1)
								res->origin = lorigin;
							else 
								res->origin = currentVal->origin;
						} 
						else {
							res = initShadowValue((UWord)addr);
							if (clo_simulateOriginal) {
		...
		// else if (currentVal)
		// {
		// 	VG_(tool_panic)("have unhandled operation in isOpFloat\n");
		// }
	...
	else if ((Int)isFloat == 2)
	{
		UInt* orgUi = (UInt*)addr;
		
		ShadowValue* preVal = VG_(HT_lookup)(globalMemory, addr-4);
		
		
		if(preVal&&(preVal->opCount>0)&&(*orgUi>0x100000))
		{
			// const HChar* lastOperation = VG_(describe_IP)(lorigin, NULL);
			// VG_(umsg)("(Bit) Last operation: %s+1\n", lastOperation);
			// VG_(printf)("1902 t %X  %X\n", addr-4,*orgUi); 
			_mpfr_sign = ((*orgUi>>31)!=1?1:0xFFFFFFFF);//compu sign 
			exptmp= (*orgUi>>20)&0x7FF;
			_mpfr_exp= (exptmp-0x3FF+1);//compu exp 
			_mpfr_prec= clo_precision;	
			_mpfr_dl = preVal->value->_mpfr_d[pos]<<21;//save low 32 
			dtemp=((*orgUi&0xFFFFF)+0x100000);
			_mpfr_d0=(dtemp<<(cell_bit-21))+(_mpfr_dl>>21);//compu high and rewrite low 32 
			preVal->value->_mpfr_sign=_mpfr_sign;//set sign
			preVal->value->_mpfr_exp=_mpfr_exp;//set exp
//				preVal->value->_mpfr_prec=_mpfr_prec;//set prec
			preVal->value->_mpfr_d[pos]=_mpfr_d0;//set significant
			preVal->isConst=True;
		}
//		else if(currentVal && (currentVal->opCount>0))
//		{
////			VG_(printf)("addr %X us %X\n", addr,t);
//			_mpfr_hl=currentVal->value->_mpfr_d[pos]>>(cell_bit-21);
//			ltemp=(t<<(cell_bit-53));
//			_mpfr_dl0=ltemp+(_mpfr_hl<<((cell_bit-21)));
//			currentVal->value->_mpfr_d[pos]=_mpfr_dl0;
//			currentVal->isConst=True;
//		}	 
		
	}
	else if ((Int)isFloat == 3)//���� �洢32bit���� 
	{
		ShadowValue* preVal = VG_(HT_lookup)(globalMemory, addr-4);
		
		if(preVal && (preVal->opCount>0)&&(t!=0))
		{
			
			_mpfr_sign = ((tmp>>31)!=1?1:0xFFFFFFFF);//compu sign 
			exptmp= (tmp>>20)&0x7FF;
			_mpfr_exp= (exptmp-0x3FF+1);//compu exp 
			_mpfr_prec= clo_precision;	
			_mpfr_dl = preVal->value->_mpfr_d[pos]<<21;//save low 32 
			dtemp=((tmp&0xFFFFF)+0x100000);
			_mpfr_d0=(dtemp<<(cell_bit-21))+(_mpfr_dl>>21);//compu high and rewrite low 32 
			preVal->value->_mpfr_sign=_mpfr_sign;//set sign
			preVal->value->_mpfr_exp=_mpfr_exp;//set exp
//			preVal->value->_mpfr_prec=_mpfr_prec;//set prec
	...
	else if (((Int)isFloat == 4))
	{
		
		Double* orgDb = (Double*)addr;
		if((t>0x100000)&&(t<0xFFFFFFFF)) 
		{
			if (currentVal) 
			{
				res = currentVal;
...
//			updateMeanValue(addr, Iop_64to32, &(res->value), 0, res->origin, 0, 0);
		}
...
static void instrumentStore(IRSB* sb, IRTypeEnv* env, IRStmt* store, Int argTmpInstead) {
	...
	if (data->tag == Iex_RdTmp) {
		...
		if(typeOfIRTemp(env, data->Iex.RdTmp.tmp)==Ity_I32)
		{
			isFloat=2;	
		}
	...
	{
		if(data->Iex.Const.con->tag == Ico_U32)
		{
			isFloat= 3;
			num =(UInt)data->Iex.Const.con->Ico.U32;
		}
		else if(data->Iex.Const.con->tag == Ico_U64)
		{
			num =(ULong)data->Iex.Const.con->Ico.U64>>32;
			isFloat= 4;
		} 
...
static void setHigh(ULong addrFp, UInt a)
{
	...
	if(svalue)
	{
		...
		mpfr_set_d(svalue->value,x,MPFR_RNDN);
	}
}
static void showShadowValue(Char *FpName, ULong addrFp)
{
	ShadowValue* svalue = VG_(HT_lookup)(globalMemory, addrFp);
	if(svalue)
	{
		...
		unsigned char i;
		unsigned char *j;
		j=(unsigned char *)&(svalue->value);
		VG_(umsg)("%s`s Addr is %X Shadow Value Addr is %X and Corresponding content is \n%s \n",FpName,addrFp,&(svalue->value),mpfrBuf);
		for(i=0;i<sizeof(svalue->value);i++)
			VG_(umsg)("%x[%d] ",j[i],j[i]);
			VG_(umsg)("\n\n");
	}
	else
	{
		VG_(umsg)("%s No Shadow Value!\n",FpName);
	}
}
void getShadowStr(Char **s,ULong addr, mp_exp_t *e,mpfr_sign_t *sign)
{
	ShadowValue* svalue = VG_(HT_lookup)(globalMemory, addr);
	if(svalue)
	{
		*sign=svalue->value->_mpfr_sign;
		mpfr_abs(arg3tmpX,svalue->value, STD_RND);
		*s = mpfr_get_str (NULL, &(*e), 10, 0, arg3tmpX, STD_RND);
	}
}
...
static void mapOrigin(ULong addrFp) {
	ShadowValue* svalue = VG_(HT_lookup)(globalMemory, addrFp);
	if (svalue) {
		if (svalue->orgType == Ot_FLOAT) {
			Float* orgFl= (Float*)addrFp;
			if(abs((mpfr_get_flt(svalue->value, STD_RND))-(*orgFl))/(abs(mpfr_get_flt(svalue->value, STD_RND))>1.0e-30?abs(mpfr_get_flt(svalue->value, STD_RND)):1.0e-30)<1.0e-3)
			*orgFl=mpfr_get_flt(svalue->value, STD_RND);
			
		} else if (svalue->orgType == Ot_DOUBLE) {
			Double* orgDb = (Double*)addrFp;
			if(abs((mpfr_get_d(svalue->value, STD_RND))-(*orgDb))/(abs(mpfr_get_d(svalue->value, STD_RND))>1.0e-30?abs(mpfr_get_d(svalue->value, STD_RND)):1.0e-30)<1.0e-3)
...
/*�¶��庯��ShadowValueToFloat�𵽽�ԭʼ�����������ַΪaddrFp����shadow �Ĵ�����ֵȡ����������ַΪaddrTempFp���ĸ�������У�����ͷ�ԭ�е�ShadowValue��*/
static void shadowValueToFloat(ULong addrFp,ULong addrTempFp)//��������������1����ʾԭʼ�����������ɱ�����ַ 
{
	ShadowValue* svalue = VG_(HT_lookup)(globalMemory, addrFp);
	if (svalue) {//shadow value��ֵ���ǿ�
		if (svalue->orgType == Ot_FLOAT) {
			Float* orgFl = (Float*)addrTempFp;
			*orgFl = mpfr_get_flt(svalue->value, STD_RND);
		} else if (svalue->orgType == Ot_DOUBLE) {
			Double* orgDb = (Double*)addrTempFp;
			*orgDb = mpfr_get_d(svalue->value, STD_RND);
		} else {
			tl_assert(False);
		}
	}
    else//shadow valueΪNULL
    {
        Double* orgDa = (Double*)addrFp;
        Double* orgDb = (Double*)addrTempFp;
        *orgDb= *orgDa;
    }
    freeShadowValue(svalue,True);//�ͷ�shadow value
}


static void reMapShadow(ULong addrFp)
{
	ShadowValue* svalue = VG_(HT_lookup)(globalMemory, addrFp);
	if (svalue) {
	VG_(umsg)("shadow value have exits!\n");
		if (svalue->orgType == Ot_FLOAT) {
			Float* orgFl = (Float*)addrFp;
...
void releaseAddArgs(ShadowValue *sv)
{
	if(sv == NULL)
	return;
	mpfr_set_d(sv->addArg1,0.0,STD_RND);
	mpfr_set_d(sv->addArg2,0.0,STD_RND);
}
...
static void writeMemoryRelError(ShadowValue** memory, UInt n_memory) {
	...
	for (i = 0; i < n_memory; i++) {
		...
		total++;
		if((memory[i]->isConst == True)){
//			VG_(printf)("3565 %X %d \n",memory[i]->origin, memory[i]->opCount);
			continue ;
		}
...
static void fd_fini(Int exitcode) {
	...
	VG_(dmsg)("OPTIMIZATION - UnOpCount:  total %'u, ignored: %'u\n", UnOpCount, UnOpCount);
	VG_(dmsg)("OPTIMIZATION - BinOpCount:  total %'u, ignored: %'u\n", dBinOpCount, BinOpCount);
	VG_(dmsg)("OPTIMIZATION - iBinOpCount:  total %'u, ignored: %'u\n", iBinOpCount, dBinOpCount);
	VG_(dmsg)("OPTIMIZATION - uBinOpCount:  total %'u, ignored: %'u\n", uBinOpCount, dBinOpCount);
...
static Bool fd_handle_client_request(ThreadId tid, UWord* arg, UWord* ret) {
	switch (arg[0]) {
		...
		case VG_USERREQ__CLEAR_STAGE:
			...
			break;
		case VG_USERREQ__ShadowValueToFloat:
			 shadowValueToFloat(arg[1], arg[2]);
			 break;
		case VG_USERREQ__ReMapShadow:
			 reMapShadow(arg[1]);
			 break;
		case VG_USERREQ__MapOrigin:
			 mapOrigin(arg[1]);
			 break;
		case VG_USERREQ__ShowShadowValue:
			showShadowValue((Char*)arg[1],arg[2]);
			break;
		case VG_USERREQ__SetHigh:
			setHigh(arg[1],arg[2]);
			break;
		...
		case VG_USERREQ__BEGIN:
			beginAnalyzing();
			break;
		case VG_USERREQ__GetShadowStr:
			getShadowStr((Char**)arg[1],arg[2],(mp_exp_t *)arg[3],(mpfr_sign_t *)arg[4]);
			break;
...
static void fd_post_clo_init(void) {
  ...
  VG_(umsg)("ignore-end=%s\n", clo_ignore_end ? "yes" : "no");
	...
	bitWrtmp = VG_(HT_construct)("Bit Wrtmp");

	cell_bit=sizeof(mp_limb_t)*8;
	pos=((clo_precision)%cell_bit==0?(clo_precision)/cell_bit:(clo_precision)/cell_bit+1)-1;
	...
	const HChar* clientName = "";
	VG_(sprintf)(pscvfname, "%s%_psc_location_special", clientName);
	getFileName(pscvfname);
	pscfileRes = VG_(open)(pscvfname, VKI_O_CREAT|VKI_O_TRUNC|VKI_O_WRONLY, VKI_S_IRUSR|VKI_S_IWUSR);
	pscfile = sr_Res(pscfileRes);
	// writeWarning(pscfile);
...
static void fd_pre_clo_init(void) {
   	...
   	VG_(details_copyright_author)("Copyright (C) 2010-2017 by Florian Benz &Shi Jia.");
